...
public class ContainerElectricRefrigeratedChest extends Container
{
    private TileEntityElectricRefrigeratedChest tileEntity;
    private IInventory itemInventory;
    private boolean isBlock;

    public ContainerElectricRefrigeratedChest(InventoryPlayer inventory, TileEntityElectricRefrigeratedChest tentity, IInventory inv, boolean b)
    {
        tileEntity = tentity;
        itemInventory = inv;
        isBlock = b;
        ...
        for(slotX = 0; slotX < 3; ++slotX)
        {
            ...
            {
                addSlotToContainer(new SlotElectricRefrigeratedChest(inventory, slotY + slotX * 9 + 9, 8 + slotY * 18, 148 + slotX * 18));
            }
        }

        for(slotX = 0; slotX < 9; ++slotX)
        {
            addSlotToContainer(new SlotElectricRefrigeratedChest(inventory, slotX, 8 + slotX * 18, 206));
        }
    }
    ...
    public ItemStack slotClick(int slotNumber, int destSlot, int modifier, EntityPlayer player)
    {
        if(modifier == 2 && destSlot >= 0 && destSlot < 9)
        {
            ...
            if(BlockMachine.MachineType.get(itemStack) == BlockMachine.MachineType.ELECTRIC_REFRIGERATED_CHEST)
            {
                return null;
            }
        }
