...
public final class MekanismUtils
{
	...
	public static void openElectricRefrigeratedChestGui(EntityPlayerMP player, TileEntityElectricRefrigeratedChest tileEntity, IInventory inventory, boolean isBlock)
	{
		...
		if(isBlock)
		{
			Mekanism.packetHandler.sendTo(new ElectricRefrigeratedChestMessage(ElectricRefrigeratedChestPacketType.CLIENT_OPEN, true, false, 0, id, Coord4D.get(tileEntity)), player);
		}
		else {
			Mekanism.packetHandler.sendTo(new ElectricRefrigeratedChestMessage(ElectricRefrigeratedChestPacketType.CLIENT_OPEN, false, false, 0, id, null), player);
		}

		player.openContainer = new ContainerElectricRefrigeratedChest(player.inventory, tileEntity, inventory, isBlock);
		player.openContainer.windowId = id;
		player.openContainer.addCraftingToCrafters(player);
	}
